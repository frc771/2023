...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    m_Claw.setDefaultCommand(new ClawKill(m_Claw));
  ...
  private void configureBindings() { //Used to say configureButtonBindings
    Y1.whileTrue(new ClawUp(m_Claw)); //replaced whileHeld with whileTrue... might need to be used with RepeatCommand
    A1.whileTrue(new ClawDown(m_Claw));
    TR.whileTrue(new ClawKill(m_Claw));
    B1.whileTrue(new IntakeIn(m_Intake));
    X1.whileTrue(new IntakeOut(m_Intake));
    TR.whileTrue(new IntakeKill(m_Intake));
